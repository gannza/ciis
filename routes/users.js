...
router.get('/update/:id', function(req, res, next) {
	var db = req.con;
	db.query('SELECT * FROM students WHERE st_code = ? ',[req.params.id] ,function(err,rows){
		if(err){
...
router.put('/update', function(req, res, next) {
	...
	db.query('UPDATE students set ? WHERE st_code = ?',[data, req.body.id] ,function(err,rows){
		if(err){
...
router.delete('/delete/:id', function(req, res) {
	...
	var db = req.con;
	db.query('DELETE FROM students WHERE st_code = ? ',[req.params.id] ,function(err,rows){
		if(err){
			res.send(JSON.stringify({'status': 0, 'msg': 'Error User deleted', 'raw': JSON.stringify(req.params)}));
		}
...
router.post('/addUser', function(req, res, next) {
	var db = req.con;
	console.log("FormData "+ JSON.stringify(req.body.name));
  if(req.body.name==="" || req.body.email===""){
    res.render('userAdd', { userAdd_empty:'Fill empty fields'});
  console.log("mpty");
  }else{
	var qur = db.query('INSERT INTO students set ? ', req.body , function(err,rows){
		//if(err) throw err;
